...
goog.scope ->
  ...
  # todo: try coffeescript syntax

  _.REMOVE_DIACRITICS_MAP = [
  ...
  _.removeDiacritics = (str) ->
    key = '_' + str
    cached = _.removeDiacriticsCache[key]
    return cached if cached
    for item in _.REMOVE_DIACRITICS_MAP
      str = str.replace item.letters, item.base
    _.removeDiacriticsCache[key] = str

  ###*
    @type {Object.<string, string>}
  ###
  _.trimLowerAndRemoveDiacriticsCache = {}

  ###*
    @param {string} str
    @return {string}
  ###
  _.trimLowerAndRemoveDiacritics = (str) ->
    key = '_' + str
    cached = _.trimLowerAndRemoveDiacriticsCache[key]
    return cached if cached
    str = _.removeDiacritics goog.string.trim(str).toLowerCase()
    _.trimLowerAndRemoveDiacriticsCache[key] = str
