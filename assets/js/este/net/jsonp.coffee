###*
  ...
  Servers may signal errors with a second callback argument. This resemblers an HTTP
  status code that would be used if the response was not JSONP. If such error is signalled
  and the status is different from 200, opt_errorCallback is invoked rather than opt_replyCallback.
  ...
  This is needed as all JSONP responses must be sent with 200 HTTP status code, hence
  there is no ability for goog.net.Jsonp to recognize the error. Signalling errors through payload 
  is quite cumbersome, so we rather signal error through a second callback argument.
...
###
este.net.Jsonp = (uri, opt_callbackParamName) ->
  goog.base @, uri, opt_callbackParamName

goog.inherits este.net.Jsonp, goog.net.Jsonp

goog.scope ->
  `var _ = este.net.Jsonp`
  ...
  ###
  _::send = (opt_payload, opt_replyCallback, opt_errorCallback, opt_callbackParamValue) ->
    goog.base @, 'send', opt_payload, (data, status) ->
